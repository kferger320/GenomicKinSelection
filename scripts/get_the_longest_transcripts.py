##retrieved from https://www.zybuluo.com/lakesea/note/1743213
...
annotated=sys.argv[1] #"annotated" or "unannotated"
...
for line in fileinput.input():
    ...
    if content[2] == "transcript" or content[2] == "mRNA":
        ...
        if annotated == "annotated":
            tx_parent = re.search(r'Parent=gene-(.*?)[;\n]',content[8]).group(1)
        
        else: #formatting differences inn augustus annotations
        ...
        tx_pos_dict[tx_id] = [content[0], content[1], content[2], content[3], content[4], content[5], content[6], content[7], content[8].strip() ]
...
for gene, txs in gene_dict.items():
    ...
    # print(tx_pos_dict[lst_tx])
    tx_lis = [tx_pos_dict[lst_tx][i] for i in range(9)]
